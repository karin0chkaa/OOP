...
class Matrix {
  ...
  static List<List<double>>? parse(List<String> matrixLines) {
    ...
    try {
      ...
      for (int i = 0; i < 3; ++i) {
        List<String> matrixRowElements =
            matrixLines[i].trim().split(RegExp(r'\s+'));
        if (matrixRowElements.length != 3) {
          return null;
        }

        List<double> row = [];
        for (String element in matrixRowElements) {
          try {
  ...
  static List<List<double>> multiplyMatrices(
      List<List<double>> leftMatrix, List<List<double>> rightMatrix) {
